    ...
    try:
        ...
        for line in all_lyrics:
            ...
            if analysis["parentheticals"]:
                results.append({
                    "line": line,
                    "line_without_parentheses": analysis["line_without_parentheses"],
...
async def main():
    ...
    for song_dir in songs_dir.iterdir():
        if song_dir.is_dir():
            ...
            if output:
                logger.info(f"✅ Found {output['lines_with_parentheticals']} lines with parentheticals out of {output['total_lines']} total lines")
